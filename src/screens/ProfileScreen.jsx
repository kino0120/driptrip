...
export default function ProfileScreen({ navigation }) {
  ...
  return (
    <FlatList
      ...
      data={currentData}
      keyExtractor={(item, i) => item.id ?? item.post_id ?? String(i)}
      ListHeaderComponent={
        <View>
          <View style={styles.header}>
            <View style={styles.avatar}><Text style={styles.avatarText}>{(profile?.display_name || profile?.username || user?.email || '?')[0].toUpperCase()}</Text></View>
